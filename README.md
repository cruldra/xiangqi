...
- [x] **1.4** [P0] 创建 `Board` 场景 - 木纹背景、使用 `_draw()` 绘制网格线
- [x] **1.5** [P0] 创建 `EventBus` 单例 - 核心信号定义
- [x] **1.6** [P0] 添加中文字体资源 - 支持繁体字符 (帥仕相馬車砲兵等)
...
- [x] **2.1** [P0] 创建 `Piece` 场景 - 圆形背景 + Label显示汉字
- [x] **2.2** [P0] 实现 `BoardLogic.setup_initial_position()` - 初始棋子布局
- [x] **2.3** [P0] 创建棋子生成系统 `Board.gd` - 根据BoardLogic状态实例化棋子
- [x] **2.4** [P1] 设置棋子样式 - 红/黑颜色区分、合适的尺寸
- [x] **2.5** [P0] 创建 `main.tscn` - 整合Board + 基本UI结构
...
- [x] **3.1** [P0] 实现 `MoveValidator.validate_general()` - 九宫内直线移动
- [x] **3.2** [P0] 实现 `MoveValidator.validate_advisor()` - 九宫内斜线移动
- [x] **3.3** [P0] 实现 `MoveValidator.validate_elephant()` - 田字走、塞象眼、不过河
- [x] **3.4** [P0] 实现 `MoveValidator.validate_horse()` - 日字走、蹩马腿
- [x] **3.5** [P0] 实现 `MoveValidator.validate_chariot()` - 直线移动
- [x] **3.6** [P0] 实现 `MoveValidator.validate_cannon()` - 直线移动 + 隔子吃
- [x] **3.7** [P0] 实现 `MoveValidator.validate_soldier()` - 前进 + 过河后横移
- [x] **3.8** [P0] 创建 `MoveGenerator.get_valid_moves(piece, pos)` - 整合所有验证器
...
- [x] **4.1** [P0] 实现棋子点击检测 `Piece.gd` - 使用Area2D
- [x] **4.2** [P0] 创建 `GameManager` 单例 - 协调游戏流程
- [x] **4.3** [P0] 实现棋子选中高亮 - 视觉反馈
- [x] **4.4** [P0] 实现合法走法指示器 - 棋盘上显示可移动位置圆点
- [x] **4.5** [P0] 实现走棋执行 - 点击目标位置完成移动
- [x] **4.6** [P0] 实现回合切换 - 走棋后自动换方
- [x] **4.7** [P0] 防止选择对方棋子 - 只能选当前方棋子
...
- [x] **5.1** [P0] 实现 `CheckDetector.is_in_check(side)` - 检测是否被将军
- [x] **5.2** [P0] 实现将帅照面规则检测 - 两将不能在同一列无遮挡
- [x] **5.3** [P0] 过滤导致被将军的走法 `MoveGenerator.filter_check_moves()`
- [x] **5.4** [P0] 实现 `CheckDetector.is_checkmate(side)` - 无合法走法且被将军
- [x] **5.5** [P1] 实现 `CheckDetector.is_stalemate(side)` - 无合法走法但未被将军
- [x] **5.6** [P1] 添加将军警告视觉反馈 - UI提示"将军！"
- [x] **5.7** [P0] 添加游戏结束检测和显示 - 胜负对话框
...
- [x] **6.1** [P0] 创建 `StatusPanel` - 显示当前回合 (红方/黑方)
- [x] **6.2** [P1] 创建 `MoveHistoryPanel` - 可滚动的走棋历史列表
- [x] **6.3** [P1] 实现中文记谱法 - 如"炮二平五"、"馬八進七"
- [x] **6.4** [P0] 添加"新游戏"按钮功能 - 重置棋盘
- [x] **6.5** [P1] 实现走棋历史追踪 `BoardLogic` - Array[Move]
- [x] **6.6** [P1] 实现悔棋功能 - 恢复上一步状态
- [x] **6.7** [P1] 添加"悔棋"按钮到UI
...
- [x] **7.1** [P1] 添加棋子移动动画 - 使用Tween
- [x] **7.2** [P1] 添加吃子动画效果 - 粒子或缩放
- [x] **7.3** [P1] 响应式棋盘缩放 - 适配窗口大小
- [x] **7.4** [P2] 添加棋子悬停效果 - 鼠标悬停反馈
- [x] **7.5** [P2] 创建游戏主题资源 - 统一视觉样式
- [x] **7.6** [P2] 添加键盘导航支持 - 无鼠标操作
- [x] **7.7** [P2] 高亮上一步走法 - 棋盘标记
